...
.import _mneml, _mneml_d, _mnemr, asm_entry
.include "symbols.inc"
.export list2,monz,getlnv,prntyx,crout,getnum,zmode,cout,bell,_reset,_stop
...
_exit:           jmp $FFFF
                ;
                ; Stop execution until reset happens
                ;
_stop:           stp
                ;
                ; getline vector.
...
getfmt:         tax
                ...
                bne @mnndxc1
                lda #<(_mneml_d-_mneml)  ; one mnemonic, last one.
                rts
...
@prntbl:        jsr prbl2
                ...
                cpy #$04
                bcc @prntbl
                ldy opcode          ; recover mnemonic index
                lda _mneml,y
                sta lmnem           ; fetch 3 char mnemonic
                lda _mnemr,y         ;   (packed in 2 bytes, only chars A-Z
                sta rmnem
prmn1:          lda #$00
                ldy #$05
...
tosub:          tya
                ...
                tax                 ; to much fumbling making them all stay on page $FE
...
; ' '           add preceding command to line
                ...
                ;
subtbl:         .addr asm_entry         ; ! - Enter assembler
                .addr _exit             ; Q - Quit emulator
                .addr bascont           ; CTL-C - exit moitor to installed language
                .addr usr               ; CTL-Y - execute user command at vector %3F8
                .addr regz              ; CTL-E - examine registers
                .addr trace             ; "T" - Trace until reset or brk
                .addr vfy               ; "V" - Verify memory range
                .addr inprt             ; [0-7] CTL-K - input from keyboard (0) or peripheral card 1-7
                .addr stepz             ; "S" - Step
                .addr outprt            ; [0-7] CTL-P - send output to video (0) or printer peripheral 1-7
                .addr x_lang            ; CTL-B -
                .addr setmode           ; "-" - hexadecimal substraction
                .addr setmode           ; "+" - hexadecimal addition
                .addr move              ; "M" - mpve memory range
                .addr lt                ; "< aaaa.bbbb M|V" - Move / compare memory range
                .addr setnorm           ; "N" - set to normal output
                .addr setinv            ; "I" - set to inverse output
                .addr list              ; "L" - list preceding memory add
                .addr write             ; "aaaa.bbbb W" - Save memory range on tape
                .addr go                ; "aaaa G" - Run program as subroutine at aaaa
                .addr read              ; "aaaa.bbbb R" - Read memory range from tape
                .addr setmode           ; colon xx yy... - change current location. to given ehx bytes
                .addr setmode           ; ".yyyy" - examine bytes between after current position and yyyy
                .addr crmon             ; enter - starts evaluation of commands in input line
                .addr blank             ; space separator
